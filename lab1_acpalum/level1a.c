/* moves NUM_DRONES from starting positions from input .txt file
    to destination position from input .txt file. It then returns
    the drone to the starting position once destintion is reached...
    Collision with other drones is detected and avoided */
...
/*
 * Name: moveWest
 * Description: moves drone west on the grid a depending on where there is or isn't a collision
 */
...
int main(int argc, char *argv[]) {
  ...
  FILE *ptr_file;
  if (argc < 2) {
    printf("Too few arguments. usage level1a <delivery file>\n");
    return -1;
  }
